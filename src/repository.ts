...
export class Repository implements Disposable {
	...
	constructor(
		...
		onSuccessfulPush(() => {
			const gitConfig = workspace.getConfiguration('vsr');
	...
	async pullFrom(rebase?: boolean, remote?: string, branch?: string, unshallow?: boolean): Promise<void> {
		await this.run(Operation.Pull, async () => {
			await this.maybeAutoStash(async () => {
				const config = workspace.getConfiguration('vsr', Uri.file(this.root));
				...
				const onExit = (exitCode: number) => {
					if (exitCode === 1) {
						...
						if (/ is in submodule /.test(stderr)) {
							reject(new GitError({ stdout: data, stderr, exitCode, gitErrorCode: VsrErrorCodes.IsInSubmodule }));
						} else {
