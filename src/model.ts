...
import { APIState as State, RemoteSourceProvider, CredentialsProvider } from './api/vsr';
...
export class Model implements IRemoteSourceProviderRegistry {
	...
	constructor(readonly git: Vsr, private readonly askpass: Askpass, private globalState: Memento, private outputChannel: OutputChannel) {
	...
	private onDidChangeConfiguration(): void {
		const possibleRepositoryFolders = (workspace.workspaceFolders || [])
			.filter(folder => workspace.getConfiguration('vsr', folder.uri).get<boolean>('enabled') === true)
			.filter(folder => !this.getOpenRepository(folder.uri));

		const openRepositoriesToDispose = this.openRepositories
			.map(repository => ({ repository, root: Uri.file(repository.repository.root) }))
			.filter(({ root }) => workspace.getConfiguration('vsr', root).get<boolean>('enabled') !== true)
	...
	async openRepository(path: string): Promise<void> {
		...
		const config = workspace.getConfiguration('vsr', Uri.file(path));
		...
		try {
			...
			const dotGit = await this.git.getRepositoryDotVersionr(repositoryRoot);
	...
	private open(repository: Repository): void {
		...
		const shouldDetectSubmodules = workspace
			.getConfiguration('vsr', Uri.file(repository.root))
