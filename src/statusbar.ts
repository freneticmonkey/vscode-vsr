...
import { Branch, RemoteSourceProvider } from './api/vsr';
...
class SyncStatusBar {
	...
	constructor(private repository: Repository, private remoteSourceProviderRegistry: IRemoteSourceProviderRegistry) {
		...
		const onEnablementChange = filterEvent(workspace.onDidChangeConfiguration, e => e.affectsConfiguration('vsr.enableStatusBarSync'));
	...
	private updateEnablement(): void {
		const config = workspace.getConfiguration('vsr', Uri.file(this.repository.root));
	...
	get command(): Command | undefined {
		...
		if (HEAD && HEAD.name && HEAD.commit) {
			if (HEAD.upstream) {
				...
				command = rebaseWhenSync ? 'vsr.syncRebase' : 'vsr.sync';
