...
export class AutoFetcher {
	...
	private async onFirstGoodRemoteOperation(): Promise<void> {
		...
		if (result === yes) {
			const gitConfig = workspace.getConfiguration('vsr', Uri.file(this.repository.root));
	...
	private async run(): Promise<void> {
		while (this.enabled) {
			...
			const period = workspace.getConfiguration('vsr', Uri.file(this.repository.root)).get<number>('autofetchPeriod', 180) * 1000;
